...
from src.matlab_backend import MatlabBackend, create_backend
...
class ACSSOrchestrator:
    def __init__(
        ...
        self._matlab_backend: MatlabBackend = create_backend(
            preference=matlab_backend,
            mcp_working_folder=str(out_root),
        )
        ...
        self.simulation_agent = SimulationAgent(backend=self._matlab_backend)
    ...
    def close(self) -> None:
        """Release backend resources (e.g. MCP session)."""
        self._matlab_backend.close()

    def run(self) -> Path:
        if self.template_slx is not None and not self.template_slx.exists():
            raise FileNotFoundError(f'Template .slx not found: {self.template_slx}')
        if self.template_slx is None:
            _log.info('No template — AI will generate Simulink model from scratch')
        ...
        print(f'[run] MATLAB backend: {type(self._matlab_backend).__name__}', flush=True)
    ...
    def _publish_waveform_evolution(self, run_dir: Path, records: list[IterationRecord]) -> list[str]:
        ...
        for record in records:
            ...
            try:
                ...
                _log.debug('Failed to load waveform from %s', waveform_path, exc_info=True)
                continue
    ...
    def _publish_manual_matlab_bundle(self, run_dir: Path, records: list[IterationRecord], req: object) -> dict[str, object]:
        ...
        # Copy build_model.m if it was generated by the model builder.
        build_model_file = iter_dir / 'build_model.m'
        if build_model_file.exists():
            shutil.copy2(build_model_file, bundle_dir / 'build_model.m')

        run_script = bundle_dir / 'run_manual_matlab.m'
        if self.template_slx is not None:
            template_line = "template_slx = fullfile(pwd, 'topology_template.slx');"
            call_line = "acss_build_and_run(payload_path, out_json, template_slx);"
        else:
            template_line = "% No template — build_model.m will generate the model."
            call_line = "acss_build_and_run(payload_path, out_json);"
    ...
    def _engineer_review_iteration(
        ...
        if review_path.exists():
            try:
                ...
                _log.debug('Failed to load review from %s', review_path, exc_info=True)
                existing_review = None
    ...
    def _load_waveform_report(self, iter_dir: Path) -> dict | None:
        ...
        try:
            ...
            _log.debug('Failed to load waveform report from %s', report_path, exc_info=True)
            return None
...
class _ProgressReporter:
    ...
    def start_run(self, name: str, run_dir: Path, template_slx: Path | None, workflow_mode: str) -> None:
        print(f'[run] Starting ACSS for {name}')
        print(f'[run] Output: {run_dir}')
        if template_slx is not None:
            print(f'[run] Template: {template_slx}')
        else:
            print(f'[run] Template: (auto-generate)')
